    ...
    <div className="p-10 -mt-2 bg-gray-900 text-white ">
      <Formik
        validationSchema={object({
          fullName: string().min(3, 'Your name must be at least 3 characters').required('Name is required!'),
          initialInvestment: number().min(100, 'Initial Investment is required! ( minimum 100 )'),
          dependents: number().min(0, 'Dependents is required! min-0 & max-5').max(5),
          acceptedTermsAndConditions: boolean().isTrue('If you agree our terms and conditions then you can create your account!'),
          investmentRisk: array(string().oneOf(['Low', 'Medium', 'High'])).min(1, 'Investment Risk must be at least one!'),
          commentAboutInvestmentRisk: mixed().when('investmentRisk', {
            is: (ir: string[]) => ir.find((ir) => ir === 'High'),
            then: string().min(10, 'Comment about investment risk must be at least 10 characters').required('Comment About Investment required! for Highly Risk!'),
        ...
        {({ values, errors, touched, isSubmitting, isValid }) => (
          <Form
            className="md:w-3/4 w-full bg-gray-800 md:p-10 p-5 rounded mx-auto"
          >
            {values.fullName && isValid ? <p
              className="bg-green-600 text-white px-5 py-1 mb-1 inline-block rounded right-1">
           ...
           <div className="bg-gray-900 p-3">
            ...
            <p className="text-xl mb-1 text-gray-200">
              Comment About Investment Risk {values.investmentRisk.includes('High') ? ' *':''}
            </p>
            <Field
           ...
           <p className="text-xl mb-1 text-gray-200">Select Dependents *</p> 
            ...
            <div className="flex justify-start mb-10">
             <FieldCheckbox name="acceptedTermsAndConditions" valueForTerms="I accept terms and conditions!" /> 
            </div> 
            ...
            <pre className="mt-10 bg-gray-900 p-4 text-blue-400 rounded-xl"> {JSON.stringify({values}, null, 2)} </pre>
